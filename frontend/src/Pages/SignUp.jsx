    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.role === "company") {
                payload = {
                    name: formData.name, // FIXED: Added name
                    companyName: formData.department, // IMPORTANT
                    email: formData.email,
                    password: formData.password,
                };
            }
            ...
            // CAMPUS
            if (formData.role === "campus") {
                payload = {
                    name: formData.name, // FIXED: Added name
                    campusName: formData.department,
                    email: formData.email,
                    password: formData.password,
                };
            }
            ...
            const endpointMap = {
                student: "http://localhost:5000/api/student/register",
                company: "http://localhost:5000/api/company/register",
                campus: "http://localhost:5000/api/campus/signup", // FIXED: Changed from /register to /signup
            };

            const res = await axios.post(endpointMap[formData.role], payload);
